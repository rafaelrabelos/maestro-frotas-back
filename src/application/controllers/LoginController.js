...
async function autentica(req, res) {
  ...
  if (!email || !senha) {
    ...
    let valideRes = [
      email_check.validate(email) || `Email [${email}] invalido.`,
    ].filter((e) => e !== true);
  ...
  const user = await Model.User.findOne({ email }).select(
    "+senha +administrador +system_user +root"
  );

  if (!user) {
    return res
      .status(400)
      .send({ status: false, erros: [`(${email})Usuário não encontrado.`] });
  } else if (!user.senha) {
    return res
      .status(400)
      .send({ status: false, erros: ["Senha não retornada pela base."] });
  } else if (!(await bcrypt.compare(senha, user.senha))) {
    return res
      .status(400)
      .send({ status: false, erros: ["Senha informada é inválida."] });
  } else {
